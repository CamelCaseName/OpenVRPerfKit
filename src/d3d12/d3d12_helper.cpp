...
namespace vrperfkit {
	...
	ComPtr<ID3D12SamplerState> CreateLinearSampler(ID3D12Device *device) {
	...
	void StoreD3D12State(ID3D12DeviceContext *context, D3D12State &state) {
		...
		context->IAGetVertexBuffers( 0, D3D12_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT, state.vertexBuffers, state.strides, state.offsets );
		context->IAGetIndexBuffer(state.indexBuffer.ReleaseAndGetAddressOf(), &state.format, &state.offset);
		context->OMGetRenderTargets( D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT, state.renderTargets, state.depthStencil.GetAddressOf() );
		...
		context->CSGetShaderResources(0, D3D12_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT, state.csShaderResources);
		context->CSGetUnorderedAccessViews(0, D3D12_1_UAV_SLOT_COUNT, state.csUavs);
	}
	...
	void RestoreD3D12State(ID3D12DeviceContext *context, const D3D12State &state) {
		...
		context->IASetVertexBuffers( 0, D3D12_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT, state.vertexBuffers, state.strides, state.offsets );
		...
		context->OMSetRenderTargets( D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT, state.renderTargets, state.depthStencil.Get() );
		...
		context->CSSetShaderResources(0, D3D12_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT, state.csShaderResources);
		for (int i = 0; i < D3D12_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT; ++i) {
			if (state.csShaderResources[i])
				state.csShaderResources[i]->Release();
		}
		UINT initial = 0;
		context->CSSetUnorderedAccessViews(0, D3D12_1_UAV_SLOT_COUNT, state.csUavs, &initial);
		for (int i = 0; i < D3D12_1_UAV_SLOT_COUNT; ++i) {
			if (state.csUavs[i])
				state.csUavs[i]->Release();
		}
	}

	ComPtr<ID3D12ShaderResourceView> CreateShaderResourceView(ID3D12Device *device, ID3D12Resource *texture, int arrayIndex) {
		...
		if (td.ArraySize > 1) {
			...
			srvd.Texture2DArray.FirstArraySlice = D3D12CalcSubresource(0, arrayIndex, td.MipLevels);
		...
		ComPtr<ID3D12ShaderResourceView> srv;
		CheckResult("creating shader resource view", device->CreateShaderResourceView(texture, &srvd, srv.GetAddressOf()));
		return srv;
	}

	ComPtr<ID3D12UnorderedAccessView> CreateUnorderedAccessView(ID3D12Device *device, ID3D12Resource *texture, int arrayIndex) {
		D3D12_TEXTURE2D_DESC td;
		texture->GetDesc(&td);
		...
		if (td.ArraySize > 1) {
			...
			uavd.Texture2DArray.FirstArraySlice = D3D12CalcSubresource(0, arrayIndex, td.MipLevels);
		...
		ComPtr<ID3D12UnorderedAccessView> uav;
	...
	ComPtr<ID3D12Resource> CreateResolveTexture(ID3D12Device *device, ID3D12Resource *texture, DXGI_FORMAT format) {
		D3D12_TEXTURE2D_DESC td;
		texture->GetDesc(&td);
